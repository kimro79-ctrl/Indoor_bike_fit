...
class _WorkoutScreenState extends State<WorkoutScreen> {
  ...
  void _showGoalSettings() {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text("목표 칼로리 설정", style: TextStyle(fontSize: 16)),
        content: TextField(
  ...
  Widget _controlButtons() => Row(mainAxisAlignment: MainAxisAlignment.center, children: [_actionBtn(_isWorkingOut ? Icons.pause : Icons.play_arrow, "시작/정지", () { setState(() { _isWorkingOut = !_isWorkingOut; if (_isWorkingOut) { _workoutTimer = Timer.periodic(const Duration(seconds: 1), (t) => setState(() { _duration += const Duration(seconds: 1); _calories += 0.15; })); } else { _workoutTimer?.cancel(); } }); }), const SizedBox(width: 15), _actionBtn(Icons.refresh, "리셋", () { if(!_isWorkingOut) setState((){_duration=Duration.zero;_calories=0.0;_avgHeartRate=0;_heartRate=0;_hrSpots=[];}); }), const SizedBox(width: 15), _actionBtn(Icons.save, "저장", _handleSaveRecord), const SizedBox(width: 15), _actionBtn(Icons.calendar_month, "기록", () => Navigator.push(context, MaterialPageRoute(builder: (c) => HistoryScreen(records: _records, onSync: _saveToPrefs))))]);
...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  // 주간/월간 통계 계산 함수
  double _getPeriodTotal(int days) {
  ...
  void _deleteRecord(WorkoutRecord r) {
    showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      actions: [
        ...
        TextButton(onPressed: () {
          setState(() { widget.records.removeWhere((item) => item.id == r.id); });
          widget.onSync(); Navigator.pop(ctx);
      ...
      child: Scaffold(
        ...
        body: SingleChildScrollView(child: Column(children: [
          ...
          Padding(
            ...
            child: Row(children: [
              _statCard("주간 합계", "${_getPeriodTotal(7).toInt()} kcal", Colors.blueAccent),
              const SizedBox(width: 12),
              _statCard("월간 합계", "${_getPeriodTotal(30).toInt()} kcal", Colors.indigoAccent),
            ]),
          ),
  ...
  Widget _buildCalendarSection() => Container(
    ...
    child: TableCalendar(
      ...
      rowHeight: 38, headerStyle: const HeaderStyle(formatButtonVisible: false, titleCentered: true),
      selectedDayPredicate: (day) => isSameDay(_selectedDay, day),
      onDaySelected: (sel, foc) => setState(() { _selectedDay = sel; _focusedDay = foc; }),
      eventLoader: (day) => widget.records.where((r) => r.date == DateFormat('yyyy-MM-dd').format(day)).toList(),
      calendarStyle: const CalendarStyle(
        markerDecoration: BoxDecoration(color: Colors.orangeAccent, shape: BoxShape.circle), // ✅ 달력 스팟 표시
  ...
  Widget _buildRecordCard(WorkoutRecord r) => GestureDetector(
    onLongPress: () => _deleteRecord(r), // ✅ 길게 눌러 삭제
    child: Container(
