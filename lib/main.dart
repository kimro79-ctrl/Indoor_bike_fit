...
class _WorkoutScreenState extends State<WorkoutScreen> {
    ...
    if (mounted && hr > 0) {
      setState(() {
        ...
        if (_isWorkingOut) {
          ...
          _avgHeartRate = (_hrSpots.map((e) => e.y).reduce((a, b) => a + b) /
                  _hrSpots.length)
              .toInt();
        }
      });
            ...
            child: Column(children: [
              ...
              SizedBox(
                  ...
                  child: ElevatedButton(
                      ...
                      onPressed: () async {
                        setState(() {
                          _goalCalories = double.tryParse(controller.text) ?? 300.0;
                        });
  ...
  Widget _connectButton() => GestureDetector(
      ...
      child: Container(
          ...
          child: Text(_isWatchConnected ? "연결됨" : "워치 연결",
              style: const TextStyle(
                  color: Colors.greenAccent,
                  fontSize: 10,
                  fontWeight: FontWeight.bold))));
      ...
      Row(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        _actionBtn(Icons.refresh, "리셋", () {
          if (!_isWorkingOut) {
            ...
            _showToast("운동 중엔 리셋 불가");
          }
        }),
        const SizedBox(width: 15),
        _actionBtn(Icons.save, "저장", () async {
          if (_isWorkingOut) {
            _showToast("일시정지 후 저장하세요.");
            return;
          }
          if (_duration.inSeconds < 5) {
            _showToast("기록이 너무 짧습니다.");
            return;
          }
        ...
        _actionBtn(Icons.calendar_month, "기록", () async {
          await Navigator.push(
              ...
              MaterialPageRoute(
                  builder: (c) =>
                      HistoryScreen(records: _records, onSync: _loadInitialData)));
          _loadInitialData();
        }),
      ]);
...
class HistoryScreen extends StatefulWidget {
  ...
  const HistoryScreen({Key? key, required this.records, required this.onSync})
      : super(key: key);
  @override
...
class _HistoryScreenState extends State<HistoryScreen> {
        ...
        builder: (context) => Container(
            ...
            child: Column(children: [
              ...
              Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                const Text(" 배경 구간: 소모 칼로리별 운동 강도 가이드", style: TextStyle(fontSize: 10, color: Colors.grey))
              ])
            ])));
  }
  ...
  Widget build(BuildContext context) {
    final dailyRecords = _currentRecords
        .where((r) => r.date == DateFormat('yyyy-MM-dd').format(_selectedDay!))
        .toList();
    return Theme(
        ...
        body: SingleChildScrollView(
          ...
          GestureDetector(
              ...
              child: Container(
                  ...
                  decoration: BoxDecoration(color: const Color(0xFF607D8B), borderRadius: BorderRadius.circular(15)),
                  child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                    const Text("나의 현재 체중", style: TextStyle(color: Colors.white)),
                    Text("${_weight}kg", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18))
                  ]))),
          Padding(
  ...
  Widget _colorBtn(String label, Color color, VoidCallback onTap) => Expanded(
      child: ElevatedButton(
          style: ElevatedButton.styleFrom(backgroundColor: color, foregroundColor: Colors.white, elevation: 0, padding: const EdgeInsets.symmetric(vertical: 10), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
          onPressed: onTap,
