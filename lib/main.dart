...
class _WorkoutScreenState extends State<WorkoutScreen> {
  ...
  void initState() { 
    super.initState(); 
    _loadInitialData(); 
    // ✅ 앱 실행 시 권한 체크 (사용자님 스타일 유지)
    Timer.run(() => _requestPermissions());
  } 

  // ✅ 안드로이드 14(삼성 폰 포함) 대응 권한 요청 함수
  Future<void> _requestPermissions() async {
    // 이미 권한이 영구 거부된 경우 설정창으로 유도
    ...
    if (scanStatus.isPermanentlyDenied || connectStatus.isPermanentlyDenied || locationStatus.isPermanentlyDenied) {
      _showToast("권한이 필요합니다. 설정창에서 허용해주세요.");
  ...
  // ✅ 워치 스캔 팝업 (디자인 원안 유지, 필터만 최적화)
  void _showDeviceScanPopup() async {
    if (_isWatchConnected) return;
    await _requestPermissions(); // 권한 먼저 확인
